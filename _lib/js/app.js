const 
    ...
    $desktop  	  = $('.desktop'),
    $mobile  	  = $('.mobile'),
    loopUrl  	  = 'https://d1le1ublvpk9xn.cloudfront.net/_lib/video/loop-',
    videoUrl  	  = 'https://d1le1ublvpk9xn.cloudfront.net/_lib/video/video-',
    mvideoUrl  	  = 'https://d1le1ublvpk9xn.cloudfront.net/_lib/video/mobile/video-',
    $header  	  = $('header'),
    $logo 	 	  = $('.logo'),
    $footer  	  = $('footer'),
    $loader  	  = $('.loader'),
...
function learnMore(state) {
    ...
    if (state === 'open') {
        ...
        learnTL.to($footer, .25, {
            ...
            x: 0,
            y: '-50%'
        }, '-=.25').staggerTo($learnP, .01, {
            className: '+=reveal'
        }, 0.3, '-=.5').fromTo($visual, .35, {
            alpha: 0,
            y: -20,
            x: 20
        }, {
...
$('.discover.btn').on('click', function() {
    ...
    $(this).siblings('.discover-transition').addClass('discover-transition--active');
    setTimeout(function() {
    ...
    $back.on('click', function() {
        ...
        setTimeout(function() {
            $('.discover-transition--active').addClass('close-discover-transition').removeClass('discover-transition--active');
        }, 250);
        setTimeout(function() {
...
let videoProgress = setInterval(function() {
    ...
    if (currTime > 0 && !$currPlaying[0].paused && !$currPlaying[0].ended) {
        if (currTime > (duration / 2) && !$('.section--active .video-wrapper').hasClass('video-wrapper--not-fs')) {
            storyText('restart');
            switch (currTitle) {
                case 'who':
                    setVideoSrc('what');
        ...
        $menuItems.on('click', function(e) {
            ...
            if (targetUrl === 'music') {
                ...
                musictl.fromTo('.music h3', .5, {
                    ...
                    x: 0,
                    ease: Power1.easeInOut
                }).staggerFromTo('.music .streaming-service', .35, {
                    y: 50,
                    alpha: 0
                }, {
                    y: 0,
                ...
                TweenMax.fromTo('.bit-nav-bar-container', .5, {
                    y: 50,
                    alpha: 0
                }, {
                    y: 0,
                ...
                TweenMax.staggerFromTo('.bit-events-container > div', .35, {
                    y: 50,
                    alpha: 0
                }, {
                    y: 0,
                ...
                TweenMax.staggerFromTo('.animate-in', .35, {
                    y: 50,
                    alpha: 0
                }, {
                    y: 0,
...
$('.close-iframe').on('click', function(e){
  ...
  setTimeout(function(){
    $('#iframe-lightbox').removeClass('yt-video-playing').addClass('close-iframe-transition');
  }, 150);
  setTimeout(function(){
